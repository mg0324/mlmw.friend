...
<body>
	...
	<div id="main_log_content">
		<div id="log_main_div">
			...
			<div class="div_title">
				<red>${user.userNickName }</red> 的日志 <span class="pull-right"> <a
					href="${pageContext.request.contextPath}/log.do?action=toLogMain&userId=${user.userId}">返回日志列表</a>
				</span>
			</div>
			<div class="log_content">
				...
				<ul class="function_ul">
					<c:if test="${isPraise eq 1}">
						<li><a href="${pageContext.request.contextPath}/log.do?action=clickLogPraise&logId=${log.logId }&userId=${user.userId}">赞(${praiseNum })</a></li>
					</c:if >
					<c:if test="${isPraise eq 0}">
						<li><a href="${pageContext.request.contextPath}/log.do?action=clickLogPraise&logId=${log.logId }&userId=${user.userId}">取消赞(${praiseNum })</a></li>
					</c:if>
					<li><a href="#add_log_comment">评论</a></li>
					<c:if test="${log.author.userId==user.userId }">
						<li><a href="${pageContext.request.contextPath}/log.do?action=updateLogUI&logId=${log.logId}">编辑</a></li>
						<li><a href="${pageContext.request.contextPath}/log.do?action=deleteLog&logId=${log.logId}">删除</a></li>
					</c:if>
				</ul>
			...
			<ul class="function_ul">
				<c:if test="${isPraise eq 1}">
					<li><a href="log.do?action=clickLogPraise&logId=${log.logId }&userId=${user.userId}">赞(${praiseNum })</a></li>
				</c:if >
				<c:if test="${isPraise eq 0}">
					<li><a href="log.do?action=clickLogPraise&logId=${log.logId }&userId=${user.userId}">取消赞(${praiseNum })</a></li>
				</c:if>
					<li><a href="#add_log_comment">评论</a></li>
					<c:if test="${log.author.userId==user.userId }">
					<li><a  onclick="return confirmUpdate();"
						href="${pageContext.request.contextPath}/log.do?action=updateLogUI&logId=${log.logId}">编辑</a></li>
					<li><a onclick="return confirmDelete();"
					href="${pageContext.request.contextPath}/log.do?action=deleteLog&logId=${log.logId}">删除</a></li>
					</c:if>
				</ul>
		...
		<div id="log_comment_reply">
			<c:forEach items="${page.list }" var="comment">
				...
				<div class="comment_reply" id="comment_reply${comment.commentId }">
					<form action="${pageContext.request.contextPath}/log.do?action=addReply&logId=${log.logId}" method="post"
		...
		<center>
			<jsp:include page="../../public/pager.jsp">
				<jsp:param value="log.do?action=showLogInfo&logId=${log.logId }" name="path" />
			</jsp:include>
		</center>
		...
		<div id="add_log_comment">
			...
			<div class="comment_right">
				<form
					action="${pageContext.request.contextPath}/log.do?action=publishComment&userId=${user.userId}"
					method="post" id="logcomment">
	...
	<script type="text/javascript">
		...
		function moveTo() {
			var logId = $("#op").val();
			var logSortId = $("#move").val();
			var userId = $("#userId").val();
			document.location.href = getRootPath()+"log.do?action=moveLog&logId="
					+ logId + "&logSortId=" + logSortId + "&userId=" + userId;
		}
		...
		/*$("#logcomment").validate({
			...
			messages:{
				content:{
					maxlength:"您的评论内容不能大于50"
				}
			}
		});
		$("#reply_reply_form"+${reply.replyId }).validate({
			rules:{
				sortName:{
