	...
	<body>
		...
		<div id="add_topic_div">
			<form role="form" id="write_log_form"
				action="${pageContext.request.contextPath}/log.do?action=createLog" method="post">
			  <center><h3><b>写日志</b></h3></center>
			  <div class="form-group">
			     <span>日志标题:</span><input class="form-control" placeholder="标题" name="logTitle"  />
			  </div>
			  <div class="form-group">
			   ...
			   <div class="form-group">
			      <span>所属日志分类:</span>
			      <select name="logSortId" required="required">
			      	<c:forEach items="${logSorts }" var="logSort">
			      		<option value="${logSort.logSortId }">${logSort.sortName }</option>
			      	</c:forEach>
			  ...
			  <div class="form-group">
			      <input type="button" class="btn btn-primary btn-sm" value="发表日志" onclick="checkForm();">
